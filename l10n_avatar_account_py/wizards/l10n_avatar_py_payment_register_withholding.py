...
class PyPaymentRegisterWithholding(models.TransientModel):

    _name = 'l10n_avatar_py_payment_register_withholding'
    _description = 'l10n_avatar_py_payment_register_withholding'
    ...
    tax_id = fields.Many2one(
        ...
        domain="[('l10n_avatar_py_withholding_payment_type', '=', parent.partner_type)]"
    )
    withholding_sequence_id = fields.Many2one(related='tax_id.l10n_avatar_py_withholding_sequence_id')
    ...
    def _tax_compute_all_helper(self):
        ...
        if self.tax_id.l10n_avatar_py_tax_type in ['earnings', 'earnings_scale']:
            to_date = self.payment_register_id.payment_date or datetime.now().date()
            ...
            domain_same_period_withholdings = [
                ...
                ('parent_state', '=', 'posted'),
                ('tax_line_id.l10n_avatar_py_code', '=', self.tax_id.l10n_avatar_py_code),
                ('tax_line_id.l10n_avatar_py_tax_type', 'in', ['earnings', 'earnings_scale']),
            ...
            domain_same_period_base = [
                ('company_id', 'child_of', self.tax_id.company_id.id),
                ('parent_state', '=', 'posted'),
                ('tax_ids.l10n_avatar_py_code', '=', self.tax_id.l10n_avatar_py_code),
                ('tax_ids.l10n_avatar_py_tax_type', 'in', ['earnings', 'earnings_scale']),
                ('partner_id', '=', self.payment_register_id.partner_id.commercial_partner_id.id),
                ('date', '<=', to_date), ('date', '>=', from_date)]
        ...
        net_amount = max(0, net_amount - self.tax_id.l10n_avatar_py_non_taxable_amount)
        ...
        if self.tax_id.l10n_avatar_py_tax_type in ['earnings', 'earnings_scale']:
            ...
            if self.tax_id.l10n_avatar_py_tax_type == 'earnings_scale':
                escala = self.env['l10n_avatar_py_earnings_scale_line'].search([
                    ('scale_id', '=', self.tax_id.l10n_avatar_py_scale_id.id),
        ...
        l10n_ar_minimum_threshold = self.tax_id.l10n_avatar_py_minimum_threshold
    ...
    def _compute_base_amount(self):
        for wth in self:
            ...
            #wth.base_amount = wth.payment_register_id.amount * sum(wth.payment_register_id.line_ids.mapped('move_id.amount_total')) / sum(wth.payment_register_id.line_ids.mapped("move_id.amount_total"))
