    ...
    def _get_py_base_res_company(self):
        return {
            self.env.company.id: {
                ...
                'l10n_avatar_py_tax_base_account_id': 'py_acc_chart_91010102',
            },
        }

    @template('py_base', 'account.journal')
    def _get_py_account_journal(self):
        #company:Company = self.env['res.company'].search([('country_id', '=', 185)], limit=1)
        #partner = company.partner_id.id
        return {
            'sale': {
                "name": "Facturas preimpresas",
                "code": "0001",
                "l10n_latam_use_documents": 1,
                "l10n_avatar_py_poe_system": "FAP",
                "l10n_avatar_py_branch": 1,
                "l10n_avatar_py_dispatch_point": 1,
                #"l10n_avatar_py_address_id": partner,
